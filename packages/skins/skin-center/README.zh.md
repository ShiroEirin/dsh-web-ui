...
- 背景控制：背景遮蔽滑杆（0–100%）为画背景的皮肤在面板后加纱，另有两个按状态的高斯模糊滑杆（0–20 px）——空对话与会话有内容各一。生效的模糊通过外壳之后的固定 `backdrop-filter` 元素施加；0 完全关闭（无元素、无 GPU 开销）。只对画背景的皮肤生效；官方默认无背景。
- Wallpaper Engine 桥：卡片可把本机 Wallpaper Engine 库用作 GUI 背景。host 半区（`src/we-library.ts` + `src/we-routes.ts`）定位 WE 安装（Steam 应用 431960：Windows 注册表 + `libraryfolders.vdf` + 探测路径），扫描项目与创意工坊内容及可选手动文件夹，经同源 `/api/skin-center/we/*` 路由提供清单、媒体（Range 流式）、预览图、web 壁纸项目文件（注入 WE API shim）与场景壁纸主贴图 PNG（由 `src/pkg-extract.ts` 进程内解码 PKG/TEX，磁盘缓存）。视频壁纸用 `<video>` 渲染，web 壁纸用沙箱 `<iframe>`，场景壁纸经内置 WebGL 播放器实时渲染（2D 图层场景与 3D 模型场景按 WE 材质/着色器语义回放）；「静态帧」模式可为任意类型钉一张零动画开销的图。单张壁纸的导入会把项目复制进 `<harness-home>/skin-center/wallpapers/`，脱离 Steam 库变更也能用，并检测创意工坊原作更新。壁纸都是用户本机文件，从不上传或再分发——创意工坊内容归原作者。没有 WE 安装（如 macOS）？面板的「手动文件夹」行可把任何 `.mp4`/`.webm` 文件夹、单个壁纸项目文件夹或项目合集文件夹加为媒体库（`~` 展开为主目录）。 壁纸网格按官方三档分级筛选，源自各项目 project.json 声明的 Steam 工坊内容分级（Everyone→G、Questionable→PG-13、Mature→R18；未声明分级的条目回退到标题标记——显式 R18/R-18 或成人词→R18、PG-13/R13→PG-13），并以每页 24 条分页，支持页码导航与跳转输入。
- 旧版迁移：v2 升级后的首次启动，一次性桥（`src/legacy-bridge.ts`）读取活动 profile `cordis.patch.yml` 里已退役的 `dsh-skin` 受管段，把活动皮肤 id 迁进 v2 选择存储，并清除旧行。迁移幂等且 fail-closed（出错时旧状态原样保留）。
...
- **背景滑杆**：遮蔽（0–100%）与两个模糊半径（0–20 px），同一命名空间。
